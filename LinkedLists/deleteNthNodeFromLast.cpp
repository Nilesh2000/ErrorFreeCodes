...
/*
The idea here is to maintain two pointers, namely a reference pointer and another main pointer. Move the reference pointer n nodes from the head. Then, start moving the main pointer from the head and the reference pointer simultaneously until the reference pointer till the reference pointer reaches the end of the list. During this process, keep a track of the previous pointer of Main_Ptr as we need to set Prev_Ptr -> next to Main_ptr -> next for the deletion to occur.
*/
