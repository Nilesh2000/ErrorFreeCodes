...
int pointOfIntersection(Node *A1, Node *A2) //A1 and A2 are addresses of the head nodes of the 2 linked lists.
{
    ...
    int Diff = abs(c1 - c2); //Returns absolute difference in the length of the two lists.

    if (c1 >= c2)
        return getIntersectionNode(A1, A2, Diff);
    else
        return getIntersectionNode(A2, A1, Diff);
}

/*
The underlying principle is to traverse the bigger list from the first node till d nodes so that from here onwards both the lists have equal number of nodes. Then we can traverse both the lists in parallel till we come across a common node. Getting a common node is done by comparing the address
of the nodes.
*/
