...
int evalExp(string Exp)
{
    ...
    for(int i = 0 ; Exp[i] != '\0' ; i++) //Parse the passes Postfix expression.
    {
        if(isdigit(Exp[i])) //If the read character is a digit / operand
        {
            S.push(Exp[i] - '0'); //Push it into the stack.
        }
